...
import csv
ppPath = os.path.dirname(os.path.abspath(__file__))
logfile = open(os.path.join(ppPath, "simulation_log.csv"), "w", newline="")
log = csv.writer(logfile)
log.writerow([
    "time","bubbleID","x","y","u","v","D","Vrel","Re","Cd",
    "Fb","Fdy","Fhist","FtotY","FT","Fdx","FtotX"
])
...
def injectBubble(meanDia, stdDevDia, bubbleList, bubbleMaxID, bubbleDiaArray, bubbleXpos, bubbleInjectionTimeIndex, injectionXpos, timeIndex):
    ...
    try:
       bubbleDiaArray[newBubbleID] = getBubbleDia(meanDia, stdDevDia) #append new bubble diameter (sampled from normal distr.) to the existing array of bubble diameters
    except:
    ...
    bubbleXpos[timeIndex, newBubbleID] = injectionXpos  #append new bubble XPos to the existing array of bubble x-positions
...
def areaAverageVoidFractionWithinBounds(y_up,y_down,n_bins,timeIndex, b):
    ...
    for iBin in range(n_bins): #compute areaaverage void frac for each bin, left to right (along x-direction)
        ...
        bubblesInBound = list() #list to store bubble ID's which correspond to the given bounded area
...
"""
#time settings
dt = 1e-2 #time step for the simulations (s) -- CHOOSE AN APPROPRIATE TIMESTEP
tEnd = 30.0 #simulation end time (s) -- SPECIFIED IN THE PROBLEM 

#DONT CHANGE THIS
n_timeSteps = int(np.ceil(tEnd/dt)) #total number of time steps
times = np.linspace(0,tEnd,n_timeSteps) #list with stored values for time indices (i.e a counter for n_timesteps: for e.g. times = [1,2, 3, 4 .... n_timeSteps])
...
n_tot_bubbles = int(np.ceil(bubbleInjectionFrequency*tEnd*n_nozzles*2)) # 20% enlarged as an arbitrary value
...
#time averaging
startAverTimeIndex = int(0.3 * n_timeSteps) #skipping first time-instants (why is this done?)-- CHOOSE AN APPROPRIATE INTERVAL
...
plt.rcParams.update({'font.size': 13})
plt.rcParams["font.family"] = "serif" 
timeIndexToPlot = int(0.5 * n_timeSteps) # CHOOSE AN APPROPRIATE VALUE
for bubble in range(bubbleMaxID):
